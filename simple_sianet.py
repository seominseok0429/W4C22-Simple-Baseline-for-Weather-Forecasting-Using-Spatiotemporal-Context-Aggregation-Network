...
class sianet(nn.Module):
    def __init__(
            self,
            in_channels: int = 10,
            out_channels: int = 6*3,  ## NEW: number of time slots to predict
        ...
        self.reduce_channels = conv1(160, ## 4 = experiment / len_seq_in
    ...
    def forward(self, x):
        ...
        xs = x.shape
        x = torch.reshape(x, (xs[0], 3, 6, xs[3], xs[4]))
        return x
    @torch.jit.unused
...
if __name__ == '__main__':
    inputs = torch.randn(3, 10, 5, 256, 256) #(b, c, t, w , h)
    model = sianet()
    output = model(inputs)
    print(output.shape)
